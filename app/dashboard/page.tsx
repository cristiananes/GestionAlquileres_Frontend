    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((card) => {
          ...
          return (
            <div key={card.label} className={`${card.bg} rounded-xl p-5 border`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto bg-orange-100 text-orange-700 text-sm font-medium px-2.5 py-0.5 rounded-full">
              {data.pendingTasks}
            </span>
          </div>
            ...
            <ul className="space-y-2">
              {data.upcomingEvents.slice(0, 5).map((ev) => (
                <li key={ev.id} className="flex items-center gap-3 text-sm">
                  <span className="w-2 h-2 rounded-full" style={{ backgroundColor: ev.color || '#3b82f6' }} />
