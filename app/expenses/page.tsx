      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {items.length === 0 && <tr><td colSpan={6} className="p-6 text-center text-gray-500">Sin gastos</td></tr>}
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                <td className="p-3">{format(new Date(item.expenseDate), 'dd/MM/yyyy')}</td>
                <td className="p-3">{item.description}</td>
                <td className="p-3 text-gray-600">{item.propertyName || '-'}</td>
                <td className="p-3"><span className="bg-orange-50 text-orange-700 text-xs px-2 py-0.5 rounded-full">{item.category}</span></td>
                <td className="p-3 text-right font-medium text-red-600">${item.amount.toLocaleString()}</td>
                <td className="p-3">
